  ...
  String page = 
    "<html>"
      ...
      "<body>"
        ...
        "<button style='width:200px; height:100px; background-color:blue;' onclick='bclick(\"2\")' id='thermo1'>Thermo1</button>"
        ...
        "<h1 id='thermo1status' style='width:200px; height:100px; '>Temp1 Status: Unknown</h1>"
        ...
        "const ws = new WebSocket(\"ws://" + WiFi.localIP().toString() + ":55533\");"
         ...
         // Server messages
            "ws.addEventListener('message', function (e) {"
              ...
              "if (e.data == \"thermo1off\") { document.getElementById('thermo1status').innerHTML = 'Thermo1: OFF' ; }"
              "if (e.data == \"thermo1on\") { document.getElementById('thermo1status').innerHTML = 'Thermo1: ON' ; }"
              "document.getElementById('thermo1status').innerHTML = e.data.substr(e.data.IndexOf('-') + 1) ;" 
  ...
  switch (type) {
    ...
    case WStype_TEXT: {
      ...
      if (data == "togglered") {
        ...
        ledserver.sendTXT(num, status);
      } else if (data == "toggleT1") {
        Thermo1_ON = !Thermo1_ON; 

        //digitalRead(thermo1, Thermo1_ON);

        char status[32];
        sprintf(status, "thermo1status-%f", degC);
        ledserver.sendTXT(num, status);
      }
      break;
    }
